...
| `XAI_API_KEY` | API key for the xAI Grok Build CLI (`grok` agent, `api-key` credential type). Sent directly to xAI (`https://api.x.ai/v1`). | When credential type is `api-key` and agent type is `grok` |
